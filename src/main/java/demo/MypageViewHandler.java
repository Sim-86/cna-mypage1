...
public class MypageViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDeleveryCanceled_then_UPDATE_2(@Payload DeleveryCanceled deleveryCanceled) {
        try {
            if (deleveryCanceled.isMe()) {
                // view 객체 조회
                List<Mypage> mypageList = mypageRepository.findByOrderId(deleveryCanceled.getOrderId());
                for(Mypage mypage : mypageList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    mypage.setStatus(deleveryCanceled.getStatus());
                    // view 레파지 토리에 save
                    mypageRepository.save(mypage);
                }
            }
